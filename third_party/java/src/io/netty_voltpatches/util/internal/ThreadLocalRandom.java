...
public class ThreadLocalRandom extends Random {
    ...
    public static synchronized long getInitialSeedUniquifier() {
        ...
        if (initialSeedUniquifier == 0) {
            ...
            ThreadLocalRandom.initialSeedUniquifier = initialSeedUniquifier =
                    SystemPropertyUtil.getLong("io.netty_voltpatches.initialSeedUniquifier", 0);
        }
        ...
        for (;;) {
            ...
            if (seedUniquifier.compareAndSet(current, next)) {
                if (current == 0 && logger.isDebugEnabled()) {
                    logger.debug(String.format("-Dio.netty_voltpatches.initialSeedUniquifier: 0x%016x", actualCurrent));
                }
