 ...
 * the system property <strong>io.netty_voltpatches.noUnsafe</strong>.
 */
public final class PlatformDependent {
    ...
    private static final boolean DIRECT_BUFFER_PREFERRED =
            HAS_UNSAFE && !SystemPropertyUtil.getBoolean("io.netty_voltpatches.noPreferDirect", false);
    ...
    static {
        if (logger.isDebugEnabled()) {
            logger.debug("-Dio.netty_voltpatches.noPreferDirect: {}", !DIRECT_BUFFER_PREFERRED);
        }
     ...
     * {@code -Dio.netty_voltpatches.preferDirect} option.
     */
    ...
    private static boolean hasUnsafe0() {
        boolean noUnsafe = SystemPropertyUtil.getBoolean("io.netty_voltpatches.noUnsafe", false);
        logger.debug("-Dio.netty_voltpatches.noUnsafe: {}", noUnsafe);
        ...
        if (noUnsafe) {
            logger.debug("sun.misc.Unsafe: unavailable (io.netty_voltpatches.noUnsafe)");
            return false;
        }

        // Legacy properties
        boolean tryUnsafe;
        if (SystemPropertyUtil.contains("io.netty_voltpatches.tryUnsafe")) {
            tryUnsafe = SystemPropertyUtil.getBoolean("io.netty_voltpatches.tryUnsafe", true);
        } else {
            tryUnsafe = SystemPropertyUtil.getBoolean("org.jboss.netty.tryUnsafe", true);
        }

        if (!tryUnsafe) {
            logger.debug("sun.misc.Unsafe: unavailable (io.netty_voltpatches.tryUnsafe/org.jboss.netty.tryUnsafe)");
            return false;
        }
    ...
    private static boolean hasJavassist0() {
        boolean noJavassist = SystemPropertyUtil.getBoolean("io.netty_voltpatches.noJavassist", false);
        logger.debug("-Dio.netty_voltpatches.noJavassist: {}", noJavassist);

        if (noJavassist) {
            logger.debug("Javassist: unavailable (io.netty_voltpatches.noJavassist)");
            return false;
        }
