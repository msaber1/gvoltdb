...
public class ExecutionEngineIPC extends ExecutionEngine {
    ...
    private enum Commands {
        ...
        Commands(int id) { m_id = id; }

        int m_id;
    }

    // These "error codes" are actually command codes for upstream commands to
    // the java process, but they need to be disjoint from true error codes so
    // that methods here can distinguish an upstream command that has arrived
    // before a direct (error) response to the last downstream command.
    // The voltdbipc processing for the downstream responses to these
    // upstream commands do not need response/command codes distinct from other
    // command codes because the voltdbipc process blocks on the response to its
    // upstream command. The response must arrive as the next communication from
    // the java process. It can not be pre-empted by other downstream traffic.

    // These definitions MUST MATCH the definitions in voltdbipc.cpp
    private static final int
        ERRORCODE_NEED_PLAN = 90,                         // Upstream request for uncached plan bytes
        ERRORCODE_NEED_PROGRESS_UPDATE = 91,              // Upstream request for "long run" reporting or timeout
        ERRORCODE_NEED_DECODE_BASE64_AND_DECOMPRESS = 92, // Upstream request for decoding work
        ERRORCODE_NEED_DEPENDENCY = 93,                   // Upstream request for dependent data
        ERRORCODE_NEED_BUFFER_EXPORT = 94,                // Upstream request for buffer export
        ERRORCODE_NEED_QUEUED_EXPORT_BYTES_STAT = 95,     // Upstream request for java-maintained stats
        ERRORCODE_CRASH_VOLTDB = 99,                      // Upstream request for crash with context string

        // This is an error code used only in this module to indicate a serious
        // ipc protocol error. It is only listed in voltdbipc.cpp for completeness.
        ERRORCODE_WRONG_SERIALIZED_BYTES = 101;           // Indication of serious protocol error.
        // These definitions MUST MATCH the definitions in voltdbipc.cpp
    ...
    private class Connection {
        ...
        void sendCommand() throws IOException
        {
            ...
            // which is presumed to have been already initialized with a message:
        ...
        ByteBuffer readBytes(int size) throws IOException
        {
            ...
            while (header.hasRemaining()) {
                int read = m_socketChannel.read(header);
                if (read == -1) {
         ...
         * The response starts with a single byte indicating a return code.
        ...
        private void satisfyDependency() throws IOException
        {
            ...
            byte[] dependencyBytes = nextDependencyAsBytes(dependencyId);
            ...
            ByteBuffer message = ByteBuffer.allocate(1 + 4 + dependencyBytes.length);
        ...
        /**
         * Send the downstream response to the upstream plan fragment request.
         * @throws IOException
         */
        ...
        /**
         * Send the downstream response to the upstream progress update request.
         * @throws IOException
         */
        private void satisfyProgressUpdate() throws IOException
        {
            int batchIndex = readInt();
            int planNodeTypeAsInt = readInt();
        ...
        /**
         * Send the downstream response to the buffer export request.
         * @throws IOException
         * @throws UnsupportedEncodingException
         */
        private void satisfyBufferExport()
        ...
        /**
         * Send the downstream response to the export stats request.
         * @throws IOException
         * @throws UnsupportedEncodingException
         */
        private void satisfyQueuedExportBytesStat()
            throws IOException, UnsupportedEncodingException
        {
            int partitionId = readInt();
            String signature = readIntSizedString();
        ...
        /**
         * Send the downstream response to the buffer decode request.
         * @throws IOException
         */
        ...
        /**
         * Crash the current host by upstream request -- no response is expected.
         * @throws IOException
         * @throws UnsupportedEncodingException
         */
        private void satisfyCrashRequest()
        {
            ...
            try {
                ...
                int reasonLength = messageBuffer.getInt();
                ...
                int numTraces = messageBuffer.getInt();
        ...
        /**
         * Read some number of tables from the wire. The message is length prefixed.
         * @param tables Output array as well as indicator of exactly how many tables to read off of the wire
         * @throws IOException
         */
        private void readResultTables(VoltTable tables[]) throws IOException
        {
            int resultTablesLength = readInt();
            if (resultTablesLength <= 0) {
                return;
            }
            ByteBuffer resultTablesBuffer = readBytes(resultTablesLength);
        ...
        void throwSerializedException(int errorCode) throws IOException
        {
            int exceptionLength = readInt();//Length is only between EE and Java.
            if (exceptionLength == 0) {
                throw new EEException(errorCode);
            }
            ByteBuffer exceptionBuffer = ByteBuffer.allocate(exceptionLength + 4);
            ...
            boolean createDrReplicatedStream)
    {
        ...
        // This buffer size was arbitrarily established at 20MB after a 10MB
        // setting was found to crash for an arbitrarily scaled unit test.
        m_dataGuard = org.voltcore.utils.DBBPool.allocateDirect(1024 * 1024 * 20);
        m_dataWithLengthPrefix = m_dataGuard.b();
        // Reserve the first 4 buffer bytes to hold the length prefix.
    ...
    @Override
    public boolean release() throws EEException, InterruptedException
    {
    ...
    /**
     * implement a round-trip IPC communication which MAY be interrupted by
     * upstream traffic, but will eventually terminate normally after a
     * success code comes back for the sent command OR throw an exception
     * after an error code comes back OR crash at the request of the partner
     * process OR give up on a broken connection.
     * @throws RuntimeException
     */
    private void sendCommandForResponse()
    {
    ...
    @Override
    public boolean tick(long time, long lastCommittedSpHandle)
    {
        m_data.clear();
    ...
    @Override
    public boolean quiesce(long lastCommittedSpHandle)
    {
        m_data.clear();
    ...
    private void sendPlanFragmentsInvocation(Commands cmd,
            int numFragmentIds,
            ...
            long undoToken)
    {
        ...
        // if inputDepIds is null, serialize dummy -1 values
        if (inputDepIds == null) {
            for (int i = 0; i < numFragmentIds; i++) {
                m_data.putLong(-1);
            }
        ...
        final int INITIAL_ALLOCATION = 2048;
        BBContainer buffer = DBBPool.wrapBB(ByteBuffer.allocate(INITIAL_ALLOCATION));
        try {
            for (int i = 0; i < numFragmentIds; ++i) {
                ByteBuffer paramBuf;
                // pset can be ByteBuffer or ParameterSet instance
                if (parameterSets[i] instanceof ByteBuffer) {
                    paramBuf = (ByteBuffer) parameterSets[i];
                }
                else {
                    ParameterSet pset = (ParameterSet) parameterSets[i];
                    paramBuf = ByteBuffer.allocate(pset.getSerializedSize());
                    pset.flattenToBuffer(paramBuf);
                    paramBuf.flip();
                }
                int minimumDesired = paramBuf.limit() - paramBuf.position();
                if (buffer.b().remaining() < minimumDesired) {
                    ...
                    // Allocate and copy
                    BBContainer next = DBBPool.wrapBB(ByteBuffer.allocate(newCapacity));
                    buffer.b().flip();
                    next.b().put(buffer.b());
            ...
            long undoToken)
    {
        ...
        // Process upstream requests triggered by the downstream request for
        // fragment execution until the execution finally succeeds, sending
        // ERRORCODE_SUCCESS or fails sending some error code that does not
        // correspond to an upstream request.
        m_connection.pollForSuccess();

        // Exit from pollForSuccess requires a reply of ERRORCODE_SUCCESS.
        // This indicates that a serialized result table should follow for
        // each fragment.
        VoltTable resultTables[] = new VoltTable[numFragmentIds];
     ...
     */
    @Override
    public boolean toggleProfiler(int toggle) { return true; }


    @Override
    public byte[] loadTable(int tableId, VoltTable table, long txnId,
            long spHandle, long lastCommittedSpHandle, long uniqueId,
    ...
    throws EEException
    {
        ...
        ByteBuffer tableBytes = PrivateVoltTableFactory.getTableDataReference(table);
        if (m_data.remaining() < tableBytes.remaining()) {
            m_data.flip();
            ByteBuffer newBuffer = ByteBuffer.allocate(m_data.remaining()
            ...
            boolean interval, Long now)
    {
        ...
        for (int locator : locators) {
            m_data.putInt(locator);
        }
        sendCommandForResponse();
        try {
            ByteBuffer messageBuffer = m_connection.readMessage();
            ...
            TableStreamType streamType, List<BBContainer> outputBuffers)
    {
        ...
        try {
            // Get the count.
            int count = m_connection.readInt();
            assert count == outputBuffers.size();

            // Get the remaining tuple count.
            long remaining = m_connection.readLong();
    ...
    @Override
    public boolean processRecoveryMessage(ByteBuffer buffer, long pointer)
    {
        m_data.clear();
    ...
    @Override
    public long applyBinaryLog(ByteBuffer log, long txnId, long spHandle,
                               long lastCommittedSpHandle, long uniqueId,
