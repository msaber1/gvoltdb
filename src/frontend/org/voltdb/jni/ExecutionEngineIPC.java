...
public class ExecutionEngineIPC extends ExecutionEngine {
    ...
    // upstream commands do not need response codes distinct from other codes
    // because the downstream process blocks on the response to its command.
    // That reponse can not be pre-empted by other downstream traffic.
     ...
     * @return */
    @Override
    protected boolean loadCatalog(final long timestamp, final byte[] catalogBytes) throws EEException
    {
        m_data.clear();
    ...
    @Override
    public boolean updateCatalog(final long timestamp, final String catalogDiffs) throws EEException
    {
        m_data.clear();
    ...
    @Override
    public boolean tick(final long time, final long lastCommittedSpHandle)
    {
        m_data.clear();
     ...
     */
    @Override
    public boolean toggleProfiler(final int toggle) { return true; }
