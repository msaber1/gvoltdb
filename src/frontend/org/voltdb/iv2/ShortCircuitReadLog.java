...
//            System.err.println("update the m_lastSpTruncationHandle from " + m_lastSpTruncationHandle + " to " + spHandle);
    ...
    {
//        System.err.println(String.format("Leader %s, truncation handle %d, replica truncation handle %d",
//                Boolean.toString(isLeader), handle, m_lastSpTruncationHandle));
        if (isLeader) {
            if (msg.getSpHandle() >= handle || handle == Long.MIN_VALUE) {
                m_mailbox.send(msg.getInitiatorHSId(), msg);
            } else {
                m_shortCircuitReadSp.add(new Item(msg));
            }
            releaseShortCircuitRead(handle, IS_SP);
            return;
        }
