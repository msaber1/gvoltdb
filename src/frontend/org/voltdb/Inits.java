...
import static org.voltdb.utils.CatalogUtil.CATALOG_BUILDINFO_FILENAME;
import static org.voltdb.utils.CatalogUtil.CATALOG_FILENAME;
            ...
            if (m_rvdb.m_myHostId == m_rvdb.m_hostIdWithStartupCatalog) {
                ...
                if (m_rvdb.m_pathToStartupCatalog == null) {
                    try {
                        ...
                        String cmd = "add / clusters cluster\nadd /clusters[cluster] databases database";
                        jar.put(CATALOG_FILENAME, cmd.getBytes());
                        String bb = "4.1\n"
                                + "voltdb-4.0.2.1-159-gfc8a77d\n"
                                + "akhanzode\n"
                                + "/Users/akhanzode/projects/voltdb/examples/voter\n"
                                + "1392672650727";
                        jar.put(CATALOG_BUILDINFO_FILENAME, bb.getBytes());
                        m_rvdb.m_pathToStartupCatalog = CatalogUtil.getVoltDbRoot(m_deployment.getPaths()).getAbsolutePath() + "/catalog.jar";
                        File cfile = new File(m_rvdb.m_pathToStartupCatalog);
                        jar.writeToFile(cfile);
