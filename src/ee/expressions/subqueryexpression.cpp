...
NValue SubqueryExpression::eval(const TableTuple *tuple1, const TableTuple *tuple2) const
{
    ...
    SubqueryContext* priorSubqueryContext = exeContext->getSubqueryContext(m_subqueryId);
    ...
    if (priorSubqueryContext) {
        std::vector<NValue>& lastParams = priorSubqueryContext->accessLastParams();
        ...
        if (!paramsChanged) {
            return priorSubqueryContext->getResult();
        }
    }

    // Out of luck. Need to run the executors

    // Without guards against unsuccessful subquery execution -- throws from within
    // executeExecutionVector or from here -- potentially set up a situation where we are associating
    // (new) parameter inputs with an obsolete cached subquery result.
    // This is only an issue if the errors are recoverable in a way that does not clear out the SubqueryContexts.
    Table* outputTable = NULL;
    try {
        outputTable = exeContext->executeExecutors(m_subqueryId);
    } catch (const SerializableEEException &e) {
        VOLT_TRACE("The Executor's execution failed in subquery '%d'", m_subqueryId);
        throw;
    }
    ...
    if (priorSubqueryContext == NULL) {
        ...
        priorSubqueryContext = exeContext->setSubqueryContext(m_subqueryId, lastParams);
    }

    // Update the cached result for the current params. All params are already updated
    priorSubqueryContext->setResult(retval);
