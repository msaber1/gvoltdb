...
{
    m_table->insertTupleForUndo(m_tupleData);
}
...
{
    TableTuple tuple(m_tupleData, m_table->schema());
    tuple.setPendingDeleteOnUndoReleaseFalse();
    m_table->m_tuplesPinnedByUndo--;

    if ( ( ! tuple.isDirty() ) && m_table->deletionMustDeferToCOWContext(tuple)) {
        //Mark it pending delete and let the snapshot land the finishing blow
        tuple.setPendingDeleteTrue();
        return;
    }
    // No snapshot in progress has a claim on the tuple. just whack it
    /*
