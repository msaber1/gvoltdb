...
public class HSQLInterface {
    ...
    static boolean m_isCompilingVoltDBDML = false;
    ...
    public static HSQLInterface loadHsqldb() {
        ...
        try {
            sessionProxy = DatabaseManager.newSession(DatabaseURL.S_MEM, name, "SA", "", props, null, 0);
        } catch (HsqlException e) {
            e.printStackTrace();
        }

        // make HSQL case insensitive
        //sessionProxy.executeDirectStatement("SET IGNORECASE TRUE;", 0);
    ...
    public void runDDLCommand(String ddl) throws HSQLParseException {
        Result result = sessionProxy.executeDirectStatement(ddl, 0);
    ...
    {
        m_isCompilingVoltDBDML = true;
        Statement cs = null;

        try {
            cs = sessionProxy.compileStatement(sql, 0);
        } catch (Throwable t) {
            m_isCompilingVoltDBDML = false;
            //t.printStackTrace();
    ...
    static boolean isCompilingVoltDBDML() {
        return m_isCompilingVoltDBDML;
    }
}
